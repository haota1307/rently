...
async function createRentalsAndRooms(users) {
  ...
  for (let userIndex = 0; userIndex < users.length; userIndex++) {
    ...
    for (let rentalIndex = 0; rentalIndex < 5; rentalIndex++) {
      ...
      // Create 5 rooms for each rental
      for (let roomIndex = 0; roomIndex < 5; roomIndex++) {
        ...
        // Create RentalPost for each room
        ...
        const post = await prisma.rentalPost.create({
          data: {
            ...
            deposit: price * 0.5, // 50% of monthly rent
            pricePaid: price,
            startDate: new Date(),
...
async function main() {
  ...
  try {
    ...
    // Create rentals, rooms, and posts
    ...
    console.log('\n💡 Test the recommendation system with these room IDs:')
    ...
    console.log('\n🔥 Frontend Test:')
    console.log(
      `   Import: useRecommendations(${sampleRooms[0]?.id || 'ROOM_ID'})`
    )
